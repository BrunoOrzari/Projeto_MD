...
#include "Musica.hpp"
     ...
     bool add_musica(std::string titulo, int d  duracao_s, std::string artista, std::string genero){
        for(item:_itens){
            if item._titulo = titulo{
                return false;
            }
        }

        Musica* musica = new Musica(std::string titulo, int d  duracao_s, std::string artista, std::string genero)
